...
function getMaxNumber(...rest) {
  ...
  for (let i = 0; i < rest.length; i += 1) {
    maxNumber = rest[i] > maxNumber ? rest[i] : maxNumber;
  }
...
function canQueenCaptureKing(queen, king) {
  const isMatchX = queen.x === king.x;
  const isMatchY = queen.y === king.y;

  if (isMatchX || isMatchY) return true;
...
function convertToRomanNumerals(num) {
  ...
  if (num < 10) return primeNumbers.get(num);

  const numToString = num.toFixed();

  return (
    tens.get(Number(numToString[0])) + primeNumbers.get(Number(numToString[1]))
  );
}
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    result += i < numberStr.length - 1 ? `${word} ` : word;
  }
...
function isContainNumber(num, digit) {
  const numToString = num.toFixed();
  const digitToString = digit.toFixed();
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const sumWithoutCurrentNum = sum - arr[i];
    if (sumWithoutCurrentNum / 2 === leftSum) {
      return i;
    }
